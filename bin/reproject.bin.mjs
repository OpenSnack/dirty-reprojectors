        ...
        if (this.tState === START) {
          ...
          if (this.bytes_remaining > 0) {
            ...
            if (this.bytes_in_sequence + i > buffer.length) {
              for (var k2 = 0; k2 <= buffer.length - 1 - i; k2++) {
                this.temp_buffs[this.bytes_in_sequence][k2] = buffer[i + k2];
              }
...
var require_JSONStream = __commonJS({
  "node_modules/JSONStream/index.js"(exports) {
    var import_jsonparse = __toESM(require_jsonparse());
    var import_through = __toESM(require_through());
    var Parser = import_jsonparse.default;
    var through = import_through.default;
    ...
    exports.parse = function(path, map) {
      ...
      parser.onValue = function(value) {
        ...
        for (var k2 in this.stack)
          if (!Object.isFrozen(this.stack[k2]))
            this.stack[k2].value = null;
      };
...
var require_geojson_stream = __commonJS({
  "node_modules/geojson-stream/index.js"(exports, module) {
    var import_JSONStream = __toESM(require_JSONStream());
    var JSONStream = import_JSONStream.default;
    var open = '{"type":"FeatureCollection","features":[';
    var close = "]}";
    module.exports.parse = function(mapFunc) {
      var indexFunc;
      if (mapFunc) {
        indexFunc = function(feature2, context) {
          return mapFunc(feature2, context[1]);
        };
      }
      ...
      if (current.e) {
        if (isSubject) {
          for (i = 0, n = points2.length; i < n; ++i)
            stream.point((point2 = points2[i])[0], point2[1]);
        } else {
...
function polygonContains_default(polygon, point2) {
  var lambda = longitude(point2), phi = point2[1], sinPhi = sin(phi), normal = [sin(lambda), -cos(lambda), 0], angle3 = 0, winding = 0;
  var sum = new Adder();
  if (sinPhi === 1)
    phi = halfPi + epsilon;
  else if (sinPhi === -1)
...
function gilbertForward(point2) {
  return [point2[0] / 2, asin2(tan2(point2[1] / 2 * radians2)) * degrees2];
}
function gilbertInvert(point2) {
  return [point2[0] * 2, 2 * atan3(sin2(point2[1] * radians2)) * degrees2];
}
function gilbert_default(projectionType) {
  ...
  function gilbert(point2) {
    return projection2(gilbertForward(point2));
  }
  if (projection2.invert)
    gilbert.invert = function(point2) {
      return gilbertInvert(projection2.invert(point2));
    };
...
function square_default(project) {
  ...
  function projectSquare(lambda, phi) {
    var s = lambda > 0 ? -0.5 : 0.5, point2 = project(lambda + s * pi2, phi);
    point2[0] -= s * dx;
    return point2;
  }
  if (project.invert)
...
function gringortenRaw(lambda, phi) {
  ...
  if (abs2(lambda - halfPi2) > epsilon3)
    lambda %= halfPi2;
  var point2 = gringortenHexadecant(lambda > pi2 / 4 ? halfPi2 - lambda : lambda, phi);
  if (lambda > pi2 / 4)
    z = point2[0], point2[0] = -point2[1], point2[1] = -z;
  return point2[0] *= sLambda, point2[1] *= -sPhi, point2;
}
...
function healpixRaw(H) {
  ...
  function forward(lambda, phi) {
    var point2, phi2 = abs2(phi);
    if (phi2 > phi03) {
      var i = min(H - 1, max(0, floor((lambda + pi2) / k2)));
      lambda += pi2 * (H - 1) / H - i * k2;
      point2 = collignonRaw(lambda, phi2);
      point2[0] = point2[0] * tau2 / dx - tau2 * (H - 1) / (2 * H) + i * tau2 / H;
      point2[1] = y03 + (point2[1] - y12) * 4 * dy1 / tau2;
      if (phi < 0)
        point2[1] = -point2[1];
    } else {
      point2 = healpixLambert(lambda, phi);
    }
    point2[0] *= w2, point2[1] /= h;
    return point2;
  }
  forward.invert = function(x, y) {
    x /= w2, y *= h;
    ...
    if (y2 > y03) {
      ...
      var point2 = collignonRaw.invert(x, 0.25 * (y2 - y03) * tau2 / dy1 + y12);
      point2[0] -= pi2 * (H - 1) / H - i * k2;
      if (y < 0)
        point2[1] = -point2[1];
      return point2;
    }
...
function polyhedral_default(root, face) {
  ...
  function forward(lambda, phi) {
    var node = face(lambda, phi), point2 = node.project([lambda * degrees2, phi * degrees2]), t;
    if (t = node.transform) {
      return [
        t[0] * point2[0] + t[1] * point2[1] + t[2],
        -(t[3] * point2[0] + t[4] * point2[1] + t[5])
      ];
    }
    point2[1] = -point2[1];
    return point2;
  ...
  function faceInvert(node, coordinates) {
    var invert = node.project.invert, t = node.transform, point2 = coordinates;
    if (t) {
      t = inverse(t);
      point2 = [
        t[0] * point2[0] + t[1] * point2[1] + t[2],
    ...
    if (invert && node === faceDegrees(p = invert(point2)))
      return p;
...
function outline(stream, node, parent) {
  var point2, edges = node.edges, n = edges.length, edge, multiPoint = { type: "MultiPoint", coordinates: node.face }, notPoles = node.face.filter(function(d) {
    return abs2(d[1]) !== 90;
  ...
  for (var i = 0; i < n; ++i) {
    ...
    if (Array.isArray(edge)) {
      if (!inside) {
        stream.point((point2 = interpolate_default(edge[0], c)(epsilon3))[0], point2[1]);
        inside = true;
      }
      stream.point((point2 = interpolate_default(edge[1], c)(epsilon3))[0], point2[1]);
    } else {
      inside = false;
      if (edge !== parent)
...
function geoAlbersUsaPr() {
  const epsilon4 = 1e-6;
  let cache, cacheStream, lower48Point, alaskaPoint, hawaiiPoint, puertoRicoPoint, point2;
  ...
  function albersUsaPr(coordinates) {
    ...
    return point2 = null, (lower48Point.point(x, y), point2) || (alaskaPoint.point(x, y), point2) || (hawaiiPoint.point(x, y), point2) || (puertoRicoPoint.point(x, y), point2);
  }
  function reset() {
    cache = cacheStream = null;
    return albersUsaPr;
...
function geoAlbersUsaTerritories() {
  const epsilon4 = 1e-6;
  let cache, cacheStream, lower48Point, alaskaPoint, hawaiiPoint, puertoRicoPoint, guamMarianaPoint, americanSamoaPoint, point2;
  ...
  function albersUsaTerritories(coordinates) {
    ...
    const y = coordinates[1];
    point2 = null;
    return (lower48Point.point(x, y), point2) || (alaskaPoint.point(x, y), point2) || (hawaiiPoint.point(x, y), point2) || (puertoRicoPoint.point(x, y), point2) || (guamMarianaPoint.point(x, y), point2) || (americanSamoaPoint.point(x, y), point2);
  }
  function reset() {
    cache = cacheStream = null;
...
var streamObjectType2 = {
  Feature: function(object, stream) {
    streamGeometry2(object.geometry, stream);
  },
  FeatureCollection: function(object, stream) {
    var features = object.features, i = -1, n = features.length;
    while (++i < n)
...
var streamGeometryType2 = {
  ...
  LineString: function(object, stream) {
    streamLine2(object.coordinates, stream, 0);
  },
  MultiLineString: function(object, stream) {
    var coordinates = object.coordinates, i = -1, n = coordinates.length;
    while (++i < n)
      streamLine2(coordinates[i], stream, 0);
  },
  Polygon: function(object, stream) {
    streamPolygon2(object.coordinates, stream);
  },
  MultiPolygon: function(object, stream) {
    var coordinates = object.coordinates, i = -1, n = coordinates.length;
    while (++i < n)
      streamPolygon2(coordinates[i], stream);
  },
  GeometryCollection: function(object, stream) {
    var geometries = object.geometries, i = -1, n = geometries.length;
    while (++i < n)
...
function streamLine2(coordinates, stream, closed) {
  var i = -1, n = coordinates.length - closed, coordinate;
  stream.lineStart();
  while (++i < n)
    coordinate = coordinates[i], stream.point(coordinate[0], coordinate[1], coordinate[2]);
...
function streamPolygon2(coordinates, stream) {
  var i = -1, n = coordinates.length;
  stream.polygonStart();
  while (++i < n)
    streamLine2(coordinates[i], stream, 1);
  stream.polygonEnd();
}
function stream_default2(object, stream) {
  if (object && streamObjectType2.hasOwnProperty(object.type)) {
    streamObjectType2[object.type](object, stream);
  } else {
    streamGeometry2(object, stream);
  }
}
...
function contains_default(ring, point2) {
  var x = point2[0], y = point2[1], contains = false;
  ...
  result: function() {
    ...
    holes.forEach(function(hole) {
      ...
      polygons.some(function(polygon) {
        if (contains_default(polygon[0], point2)) {
          polygon.push(hole);
          return true;
        }
      }) || polygons.push([hole]);
...
if (argv.list) {
  console.log(Object.keys(projections_default).join("\n"));
  process.exit(0);
}
if (!argv.forward && !argv.reverse) {
  usage("Please supply either a --forward or --reverse projection (or both).");
}
process.stdin.pipe(import_geojson_stream.default.parse()).pipe(new Transform({
  transform(feature2, _, next) {
    feature2.geometry = reproject({
      forward: argv.forward,
      reverse: argv.reverse,
      projections: projections_default
    }, feature2.geometry);
    feature2.bbox = es_default(feature2);
    this.push(feature2);
    next();
  },
  objectMode: true
})).pipe(import_geojson_stream.default.stringify()).pipe(process.stdout);
